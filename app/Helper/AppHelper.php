...
class AppHelper extends Base
{
     ...
     * @return string
     */
    public function renderImageAvatar($path, $css = 'avatar-left', $size = 48)
    {
        if (empty($path)) {
            return '';
        }
        $url = $this->helper->url->href('ConfigController', 'image', array('hash' => ($path), 'size' => $size));
        return '<div class="avatar avatar-'.$size.' '.$css.'">'.'<img src="' . $url . '">'.'</div>';
    }
    ...
    public function renderImage($path)
    {
        ...
        $url = $this->helper->url->href('ConfigController', 'image', array('hash' => ($path), 'size' => 40));
        return '<img src="' . $url . '">';
    }
}
