...
class ConfigController extends BaseController
{
    ...
    /**
     * Display the application design page
     *
     * @access public
     */
    public function design()
    {
        $this->response->html($this->helper->layout->config('config/design', array(
            'logo' => ('application_logo'),
            'logoCurrent' => $this->configModel->getOption('application_logo'),
            'title' => t('Settings').' &gt; '.t('Application design'),
        )));
    }

    /**
     * Save data of application design page
     *
     * @access public
     */
    ...
    {
        $this->checkCSRFParam();
        if (! $this->request->getFileInfo('logo')['name']) {
            $this->flash->failure(t('You must select a file!'));
        } elseif (! $this->configModel->isImage($this->request->getFileInfo('logo')['name'])) {
            $this->flash->failure(t('The file is not a valid image (Only *.gif, *.jpg, *.jpeg and *.png are allowed!)'));
        } else {
            if (! $this->configModel->uploadFile('application_logo', $this->request->getFileInfo('logo'))) {
        ...
        $this->response->redirect($this->helper->url->to('ConfigController', 'design'));
    }

    /**
     * remover data of application design page
     *
     * @access public
     */
    public function removeDesign()
    {
        $filename = $this->configModel->getOption('application_logo');
        if (! empty($filename)) {
            $this->objectStorage->remove($filename);
            $this->configModel->save(array('application_logo' => null));
            $this->flash->success(t('Settings saved successfully.'));
        }        
        $this->response->redirect($this->helper->url->to('ConfigController', 'design'));
    }
    ...
    public function image()
    {
        ...
        $filename = $this->configModel->getOption($hash);
