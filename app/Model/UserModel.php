    ...
    public function prepare(array &$values)
    {
        ...
        if (isset($values['date_of_birth'])) {
            if (! empty($values['date_of_birth'])) {
                $values = $this->dateParser->convert($values, array('date_of_birth'), true);
            }
        }
        $this->helper->model->removeFields($values, array('confirmation', 'current_password'));
        $this->helper->model->resetFields($values, array('date_of_birth', 'is_ldap_user', 'disable_login_form'));
